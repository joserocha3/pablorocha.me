...
function MobileNavItem({ href, children }) {
  return (
    <li>
      <Popover.Button as={Link} href={href} className="block py-6">
        {children}
      </Popover.Button>
    </li>
  )
...
function MobileNavigation(props) {
  return (
    <Popover {...props}>
      <Popover.Button className="flex items-center px-4 py-2 text-sm font-medium rounded-full shadow-sm group bg-white/90 text-zinc-800 shadow-zinc-800/5 ring-1 ring-zinc-900/5 backdrop-blur dark:bg-zinc-800/90 dark:text-zinc-200 dark:ring-white/10 dark:hover:ring-white/20">
        Menu
      ...
      <Transition.Root>
        ...
        >
          <Popover.Panel focus className="fixed top-0 left-0 z-50 w-full h-full p-8 bg-white/80 backdrop-blur-sm dark:bg-black/80">
            <div className="flex flex-row-reverse items-center justify-between">
              <Popover.Button aria-label="Close menu" className="p-1 -m-1">
                <CloseIcon className="w-12 h-12 text-zinc-700 dark:text-zinc-400" />
              </Popover.Button>
            </div>
            <nav>
              <ul className="-my-2 text-3xl text-zinc-800 dark:text-zinc-300">
...
function NavItem({ href, children }) {
  ...
  return (
    <li>
      <Link href={href} className={clsx('relative block px-6 py-2 transition', isActive ? 'text-sky-600 dark:text-sky-300' : 'hover:text-sky-600 dark:hover:text-sky-300')}>
        {children}
      </Link>
    </li>
  )
...
function DesktopNavigation(props) {
  return (
    <nav {...props}>
      <ul className="flex px-3 text-base font-medium rounded shadow-sm bg-white/90 text-zinc-800 shadow-zinc-800/5 ring-1 ring-zinc-900/5 backdrop-blur dark:bg-zinc-800/90 dark:text-zinc-200 dark:ring-white/10">
...
function ModeToggle() {
  ...
  return (
    <button
      ...
      className="px-3 py-2 transition rounded-full shadow-sm group bg-white/90 shadow-zinc-800/5 ring-1 ring-zinc-900/5 backdrop-blur dark:bg-zinc-800/90 dark:ring-white/10 dark:hover:ring-white/20"
      onClick={toggleMode}
    >
      <SunIcon className="h-6 w-6 fill-orange-100 stroke-orange-500 transition group-hover:fill-zinc-200 group-hover:stroke-zinc-700 dark:hidden [@media(prefers-color-scheme:dark)]:fill-orange-50 [@media(prefers-color-scheme:dark)]:stroke-orange-500 [@media(prefers-color-scheme:dark)]:group-hover:fill-orange-50 [@media(prefers-color-scheme:dark)]:group-hover:stroke-orange-600" />
      <MoonIcon className="hidden h-6 w-6 fill-slate-700 stroke-slate-500 transition dark:block [@media(prefers-color-scheme:dark)]:group-hover:stroke-slate-400 [@media_not_(prefers-color-scheme:dark)]:fill-slate-400/10 [@media_not_(prefers-color-scheme:dark)]:stroke-slate-500" />
    </button>
  )
}
      ...
      >
        {isHomePage && (
          <>
            ...
            <Container className="top-0 order-last pt-3 -mb-3" style={{ position: 'var(--header-position)' }}>
              <div className="top-[var(--avatar-top,theme(spacing.3))] w-full" style={{ position: 'var(--header-inner-position)' }}>
                <div className="relative">
                  ...
                  />
                  <Avatar large className="block w-16 h-16 origin-left" style={{ transform: 'var(--avatar-image-transform)' }} />
                </div>
              </div>
            </Container>
          </>
